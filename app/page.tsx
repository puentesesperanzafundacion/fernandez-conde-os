  ...
  return (
    <div className="shell">
      <aside className="side">
        <div className="brand"><div className="logo">FC</div><div><b>FERNÁNDEZ CONDE</b><small>OS · V2.0</small></div></div>
        <nav className="nav">
        ...
        <div className="financeLink">FINANZAS<br/><b>Sistema independiente</b></div>
      ...
      <main className="content">
        ...
        {tab === "jarvis" && (
          <>
            <PageHead eyebrow="JARVIS · LECTURA" title="Consulta del despacho" description="V2 consulta la información estructurada de FC OS; todavía no modifica Finanzas ni ejecuta acciones jurídicas autónomas." />
      ...
      {modal === "task" && <Modal title="Nueva tarea o término" close={() => setModal(null)}><form onSubmit={addTask}><Field name="title" label="Título" required/><div className="grid2"><Select name="taskType" label="Tipo" options={taskTypes.map(([value,label]) => [value,label])}/><Select name="priority" label="Prioridad" options={priorities.map((item) => [item,item])}/></div><Field name="due" label="Fecha y hora" type="datetime-local"/><Select name="matter" label="Expediente" defaultValue={selectedMatter?.id ?? ""} options={[["","General"],...matters.map((matter) => [matter.id,matter.title])]}/><TextArea name="description" label="Instrucciones"/><Actions/></form></Modal>}
...
function MatterCenter(props: {
  ...
  return <>
    ...
    {tab === "seguimiento" && <section className="trackingLayout"><div className="trackingHead"><div><div className="ey">MOTOR PROCESAL</div><h2>{stages.length} etapas</h2><p>Cada etapa conserva programación, resultado, notas y siguiente actuación.</p></div><button className="secondary" onClick={props.onIncident}><AlertTriangle size={15}/> Registrar incidencia</button></div><div className="stageStack">{stages.map((stage,index) => <StageEditor key={stage.id} stage={stage} index={index} onSave={props.onSaveStage}/>)}</div></section>}
...
function StageEditor({ stage, index, onSave }: { stage: Stage; index: number; onSave: (stage: Stage, data: Record<string,string>) => void }) {
  ...
  return <form className={`stageCard ${statusClass}`} onSubmit={submit}>
    <div className="stageIndex">{index+1}</div>
    <div className="stageMain"><div className="stageTop"><div><h3>{stage.workflow_template_stages?.name}</h3><p>{stage.workflow_template_stages?.description}</p></div><SelectInline name="state" defaultValue={stage.state} options={stageStates.map(([value,label]) => [value,label])}/></div>
      <div className="stageFields"><Field name="scheduled" label="Programada" type="datetime-local" defaultValue={dateInput(stage.scheduled_at)}/><Field name="actual" label="Fecha real" type="datetime-local" defaultValue={dateInput(stage.actual_at)}/><Field name="outcome" label="Resultado corto" defaultValue={stage.outcome ?? ""}/><Field name="result" label="Resultado / determinación" defaultValue={stage.result ?? ""}/><Field name="nextAction" label="Siguiente actuación" defaultValue={stage.next_action ?? ""}/><Field name="nextActionDue" label="Fecha siguiente actuación" type="datetime-local" defaultValue={dateInput(stage.next_action_due_at)}/></div><TextArea name="notes" label="Notas de la etapa" defaultValue={stage.notes ?? ""}/><div className="stageSave"><button className="secondary" type="submit">Guardar etapa</button></div>
    </div>
  </form>;
}
